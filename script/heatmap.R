...
obj = subset(obj, 
             subset = Minor.CellType != "Undetermined")
...
celltype$Major.CellType = factor(celltype$Major.CellType, 
                                 levels = c("T/NK cells","B lymphocytes",
                                          "Myeloid cells","MAST cells",
                                          "Endothelial cells","Epithelial cells","Fibroblasts"))
celltype = celltype[order(celltype$Major.CellType), ]
celltype$Minor.CellType = factor(celltype$Minor.CellType, 
                                 levels = unique(celltype$Minor.CellType))
...
scale_x_continuous(expand = c(0, 0),
                   ...
                   labels = label1,
                   sec.axis = dup_axis(
                     breaks = seq(2, length(levels(celltype$Minor.CellType)), by = 2),
                     labels = label2)
                  ) +
...
scale_x_continuous(expand = c(0,0)) + 
geom_hline(yintercept = as.numeric(cumsum(rev(table(gene$group)[-1]/3))) + .5, color = "white")
